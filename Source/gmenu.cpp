...
void gmenu_call_proc(TMenuItem *pItem, void (*gmFunc)())
{
	TMenuItem *v2; // eax
	int v3; // ecx
	void (**v4)(); // edx
...
int gmenu_presskeys(int a1)
{
	...
	switch ( a1 )
	{
		case VK_RETURN:
			...
			{
				PlaySFX(IS_TITLEMOV);
				((void (*)(signed int))sgpCurrItem->fnMenu)(1);
			}
			return 1;
		case VK_ESCAPE:
			PlaySFX(IS_TITLEMOV);
...
void gmenu_left_right(int a1)
{
	...
	if ( sgpCurrItem->dwFlags & 0x40000000 )
	{
		...
		((void (*)(_DWORD))sgpCurrItem->fnMenu)(0);
	}
}

int gmenu_on_mouse_move(LPARAM lParam)
{
	int v2; // edx
	...
	((void (*)(_DWORD, int))sgpCurrItem->fnMenu)(0, v2);
	return 1;
}
